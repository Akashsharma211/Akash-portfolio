      ...
      >
        ...
        >
          {loop.map((item, i) => (
            <span
              key={i}
              className="mx-5 my-2 inline-flex items-center text-sm md:text-base font-bold text-zinc-200 tracking-wide whitespace-nowrap select-none"
            >
              {renderLabel(item) === "Next.js" ? (
                <>
                  <span className="text-red-500 font-black">NEXT</span>.js
                </>
              ) : (
                renderLabel(item)
              )}
