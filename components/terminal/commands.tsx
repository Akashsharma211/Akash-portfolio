...
// Lazy load the CyberBreach game
const DynamicCyberBreachGame = dynamic(() => import("./CyberBreachGame"), {
  ssr: false,
  loading: () => <div className="text-zinc-500 text-sm">Loading Cyber Breach protocol...</div>
});
...
export const commands: Record<string, CommandHandler> = {
  help: () => (
    <div className="space-y-2">
      <div className="grid grid-cols-[140px_1fr] gap-x-4 gap-y-1 font-mono text-sm">
        ...
        <div className="text-green-400">breach</div>
        <div>Infiltrate the mainframe (Cyber Breach Arcade)</div>
        <div className="text-green-400">hack</div>
        <div>Initiate system breach protocol</div>
    ...
    return (
      <div className="space-y-1">
        <div className="mt-2">
          ...
          <div>Codolio: {link(profile.socials.codolio || "https://codolio.com/profile/Sepia_erin")}</div>
          <div>Instagram: {link(profile.socials.instagram)}</div>
          <div>Twitter/X: {link(profile.socials.twitter)}</div>
          <div>Portfolio: {link(profile.socials.portfolio)}</div>
          <div>Email: {link(`mailto:${profile.contact.email_masked}`, profile.contact.email_masked)}</div>
          <div>Phone: {link(`tel:${profile.contact.phone_masked.replace(/\s+/g, '')}`, profile.contact.phone_masked)}</div>
        </div>
      </div>
    );
  },
  ...
  // breach: cyber mainframe infiltration game
  breach: (_args, env) => {
    return <DynamicCyberBreachGame onExit={() => env.run("clear")} />;
  },

  // hack: alias/easter-egg direct command for breach
  hack: (_args, env) => {
    return <DynamicCyberBreachGame onExit={() => env.run("clear")} />;
  },
